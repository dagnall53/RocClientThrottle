...
int ThrottleNameEEPROMLocation = 200;
...
void TrySerial_IO(){
  String MSGText;
    ...
    if (wifiSSID=="Router Name"){UpdateInProgress=true;Serial.println(" Forcing request for new entries as Default Router name has not been set in Secrets.h");
                                    ...
                                    display.clear();display.drawString(64, 10, "EEPROM settings required");
                                                    display.drawString(64, 25, "Use Serial port @115200");
                                                    display.drawString(64, 40, "To enter new data");
                                    display.display();
          }else{Serial.println("");
 ...
 while ((millis()<= Timestarted+3000) || UpdateInProgress) {
    ...
    if (newData == true) {TestData=receivedChars;
                          ...
                          break;
                          case 4:
                                 NameOfThisThrottle=receivedChars; newData = false;SerioLevel=5;
                                 MSGText="Name<";
                                 MSGText+=NameOfThisThrottle;
                                 MSGText+=">";
                                 display.drawString(64, 46, MSGText); display.display();
                          ...
                          case 5:
                              ...
                              if (TestData=="rrr\0"){ display.clear(); display.drawString(64, 46, "Type xxx to resume input"); display.display();Serial.println("-----------------");Serial.println("---Starting again---");Serial.println(" Type xxx again to re-start sequence");
...
String MSGText;
  delay(100);
   ...
   Serial.print ("- Trying to ");Serial.print(MSGText);Serial.print(" BrokerAddr:");Serial.println(Broker);
  ...
  _SetupOTA(NameOfThisThrottle); // now we  have set the ota update with nickname ThrottleName 
                           //Note In Arduino Ports: You will also see the IP address so you can select wich throttle of many with the same name to update OTA
...
void TestFillEEPROM(int d){
  ...
  #endif
    for(int i=0;i<=510;i++) { EEPROM.write(i,d); }
     EEPROM.commit();delay(100);// 
}
...
String read_String(char add)
{
  ...
  len=0;
  k=EEPROM.read(add);
    while((k != '\0') && (len<99) && ((len+add)<510))   //Read until null character, 
  {     
